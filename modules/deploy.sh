        ...
        case "$enable_hyperv" in
            [Yy]*)
                ...
                HYPERV_ARGS=(
                    ...
                    '--xml' "./features/hyperv/relaxed/@state=on"
                    '--xml' "./features/hyperv/vapic/@state=on"
                    '--xml' "./features/hyperv/spinlocks/@state=on"
                    '--xml' "./features/hyperv/spinlocks/@retries=8191"
                    '--xml' "./features/hyperv/vendor_id/@state=on"
                    '--xml' "./features/hyperv/vendor_id/@value=1234567890ab"
                )
                HYPERV_CLOCK_STATUS="yes"
                CPU_FEATURE_HYPERVISOR="require"
                fmtr::info "Setting Hyper-V to custom mode with spoofed vendor_id."
                break
                ;;
            [Nn]*)
